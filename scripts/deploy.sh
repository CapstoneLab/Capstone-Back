...
if [ -n "$PUBLIC_HOSTNAME" ]; then
  ...
  DOMAIN_BASE_URL="https://api.pwd.kr/${OWNER}/${REPO}"
...
GITHUB_REDIRECT_URI="${DOMAIN_BASE_URL}/auth/github/callback"
GITHUB_OAUTH_SCOPES="read:user user:email repo"
FRONTEND_REDIRECT_URL="${DOMAIN_BASE_URL}/auth/success"
JWT_SECRET="${DEPLOY_JWT_SECRET:-$(openssl rand -hex 32)}"
JWT_ALGORITHM="HS256"
JWT_EXPIRE_MINUTES="10080"
DATABASE_URL="${DEPLOY_DATABASE_URL:-}"
UBUNTU_SSH_HOST="${DEPLOY_UBUNTU_SSH_HOST:-}"
UBUNTU_SSH_PORT="${DEPLOY_UBUNTU_SSH_PORT:-22}"
UBUNTU_SSH_USER="${DEPLOY_UBUNTU_SSH_USER:-}"
UBUNTU_SSH_PASSWORD="${DEPLOY_UBUNTU_SSH_PASSWORD:-}"
UBUNTU_RUNNER_PATH="${DEPLOY_UBUNTU_RUNNER_PATH:-}"
UBUNTU_WORKING_DIR="${DEPLOY_UBUNTU_WORKING_DIR:-}"
UBUNTU_PYTHON_COMMAND="${DEPLOY_UBUNTU_PYTHON_COMMAND:-python3}"
WINDOWS_CALLBACK_BASE_URL="${DEPLOY_WINDOWS_CALLBACK_BASE_URL:-}"
ENGINE_SHARED_TOKEN="${DEPLOY_ENGINE_SHARED_TOKEN:-}"
...
# ── Ensure api.pwd.kr virtual host exists ──
...
DOMAINEOF
  nginx -t && systemctl reload nginx
  echo "Created api.pwd.kr virtual host"
fi
