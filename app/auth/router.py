...
from app.db import get_db, get_db_or_none
...
async def github_callback(
    ...
    state: str,
    request: Request,
    db: AsyncSession | None = Depends(get_db_or_none),
) -> Response:
    ...
    if db is not None:
        ...
        print("[auth] DB not available — skipping user persistence, issuing JWT from GitHub profile.")
...
async def auth_success(request: Request, token: str = "") -> HTMLResponse:
...
<p>
  ...
  &nbsp;
  <button onclick="fetch('{prefix}/auth/me',{{headers:{{Authorization:'Bearer '+document.getElementById('tk').value}}}}).then(r=>r.json()).then(j=>document.getElementById('me').textContent=JSON.stringify(j,null,2))">
    /auth/me 호출
  </button>
</p>
